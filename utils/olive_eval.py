...
class ONNXModelHandler(_ONNXModelHandler):
    def _get_ep_selection(self, inference_settings: dict[str, Any]):
        ...
        return  if session_options else None

    def prepare_session(
        self,
        inference_settings: Optional[dict[str, Any]] = None,
        device: Device = Device.CPU,
        execution_providers: Optional[str | list[str]] = None,
        rank: Optional[int] = None,
    ):
        # user provided inference_settings > model's inference_settings > default settings
        inference_settings = self.merge_inference_settings(
            inference_settings, execution_providers
        )

        session_options = inference_settings.get("session_options", {})
        ep_selection = session_options.get("ep_selection")
        if ep_selection:
            ...
            if policy is None:
                raise ValueError(
                    f"Invalid EP selection policy: {ep_selection['policy']}"
                )

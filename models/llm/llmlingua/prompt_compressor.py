...
class PromptCompressor:
  ...
  def get_structured_dynamic_compression_ratio(
    ...
    assert len(segments) == len(
      global_dynamic_rate) == len(global_dynamic_compress)
  ...
  def control_context_budget(
    ...
    self.context_idxs.append(
      [x for idx, (x, _) in enumerate(demostrations_sort)])
    ...
    for text in context:
      ...
      segs_rate = [
          float(match[0]) if match[0] else (
            float(match[2]) if match[2] else None)

...
def quant(
    model_path: Path, data_path: Path, output_dir: Path, config=QuantizationConfig()
):
  ...
  with TemporaryDirectory(prefix="quant.qdq.") as tmp_dir:
    ...
    if config.node_optimization:
      quant_config.nodes_to_exclude = (
          ...
          + [f"/model/model/layers.{i}/self_attn/Unsqueeze_2"
             for i in range(16)]
          + [f"/model/model/layers.{i}/self_attn/Unsqueeze_3"
             for i in range(16)]
      )

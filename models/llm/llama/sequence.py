...
class ForwardSequence:
    ...
    def __init__(
        ...
        mask = np.full(
            (ctx_len + 1, ctx_len), fill_value=self.mask_val, dtype=np.float32
        )
...
class SinkSequence:
    ...
    def _input_tensors(self, seq_len=1) -> InferInput:
        ...
        mask = np.full(
            (seq_len, self.ctx_len), fill_value=self.mask_val, dtype=np.float32
        )
        ...
        # shape: (batch_size, 1, seq_len, ctx_len)
        attn_mask = np.broadcast_to(
            mask.astype(np.float32), (self.batch_size, 1, *mask.shape)
        )
        return {
            "input_ids": input_ids,
...
class SinkCache:
    ...
    def update(self, new_keys: np.ndarray, new_values: np.ndarray) -> None:
        assert new_keys.shape == self.shape, (
            f"Shapes do not match: {new_keys.shape} != {self.shape}"
        )

...
def process_dataset(dataset, model_name: str, input_cols: List[str],
                    label_col: str, **kwargs):
  processor = SamProcessor.from_pretrained(model_name)

  def process_fn(x):
    inputs = processor(images=x[input_cols[0]],
  ...
  return BaseDataset(list(processed_ds), label_col=label_col)
